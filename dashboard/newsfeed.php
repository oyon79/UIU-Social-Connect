    ...
    async function showComments(postId) {
        ...
        `;
            ...
            commentsList.innerHTML = comments.map(comment => {
                ...
                const userImage = comment.user_image || 'default-avatar.png';
                const userImageUrl = userImage !== 'default-avatar.png' ? `../${userImage}` : '';
                const userInitial = (comment.user_name || 'U').charAt(0).toUpperCase();
                ...
                <div class="comment-item" style="padding: 1rem; border-bottom: 1px solid var(--gray-light);">
                    <div style="display: flex; gap: 0.75rem;">
                        <div class="avatar" style="width: 32px; height: 32px; font-size: 14px;">
                            ${userImageUrl ? `<img src="${userImageUrl}" alt="${escapeHtml(comment.user_name || 'User')}" onerror="this.parentElement.innerHTML='<span>${userInitial}</span>'" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">` : `<span>${userInitial}</span>`}
                        </div>
                        <div style="flex: 1;">
